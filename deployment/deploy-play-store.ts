 ...
 *     which exchanges the GitHub OIDC token at /tmp/gcp-oidc.jwt for short-lived GCP tokens.
...
export const PACKAGE_NAME = 'ch.rhosys.email';
...
async function diagnoseError(err: unknown, client: PublisherClient): Promise<never> {
  ...
  if (status === 401 || status === 403) {
    ...
    if (msg.includes('iam.serviceAccounts.getAccessToken')) {
      ...
      process.stderr.write(`The CI runner's GitHub repo is not authorized to impersonate\n`);
      ...
      process.stderr.write(`  resource "google_service_account_iam_member" "play_store_wif_email" {\n`);
      ...
      process.stderr.write(`    member             = "principalSet://iam.googleapis.com/\${pool}/attribute.repository/Rhosys/SES-Email-Adapter-Android"\n`);
      process.stderr.write(`  }\n\n`);
